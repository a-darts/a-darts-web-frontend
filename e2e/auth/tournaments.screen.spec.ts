...
test.describe('Tournaments Screen', () => {
    ...
    test('como ADMIN: debe mostrar los torneos en DRAFT al activar el filtro "Todos"', async ({ page }) => {
        // 1. Navegar a la pantalla de torneos (con sesión de ADMIN iniciada por el beforeEach)
        await page.goto('/tournaments');

        // 2. Activar el filtro 'Todos'
        const allButton = page.getByRole('button', { name: 'Todos', exact: true });
        await allButton.click();

        // 3. Se debe ver tanto el torneo publicado como el borrador
        await expect(page.getByText('Open Absoluto de Aragón')).toBeVisible();
        await expect(page.getByText('Liga Secreta en Desarrollo')).toBeVisible();
    });

    test('como PLAYER: no debe mostrar los torneos en DRAFT ni al activar el filtro "Todos"', async ({ page }) => {
        await page.route(`${API_BASE}/auth/me`, async (route) => {
            if (route.request().method() === 'GET') {
                await route.fulfill({
                    status: 200,
                    contentType: 'application/json',
                    body: JSON.stringify(MOCK_USER_PLAYER),
                });
            } else {
                await route.continue();
            }
        ...
        await page.route(`${API_BASE}/tournaments`, async (route) => {
            if (route.request().method() === 'GET') {
                const publicTournaments = MOCK_TOURNAMENTS.filter(t => t.status !== TournamentStatus.DRAFT);
        ...
        // 1. Forzar al frontend a reconocer la nueva sesión simulando una recarga o nueva navegación
        await page.goto('/tournaments');
        ...
        // 3. El torneo publicado se debe ver, pero el DRAFT debe estar estrictamente oculto para el jugador
        await expect(page.getByText('Open Absoluto de Aragón')).toBeVisible();
        await expect(page.getByText('Liga Secreta en Desarrollo')).not.toBeVisible();
    });

    test('como INVITADO (sin sesión): no debe mostrar los torneos en DRAFT al activar el filtro "Todos"', async ({ page }) => {
        await page.route(`${API_BASE}/auth/me`, async (route) => {
            if (route.request().method() === 'GET') {
                await route.fulfill({
                    status: 401,
        ...
        await page.route(`${API_BASE}/tournaments`, async (route) => {
            if (route.request().method() === 'GET') {
                ...
                await route.fulfill({
                    status: 200,
                    contentType: 'application/json',
                    body: JSON.stringify({ status: "success", data: publicTournaments }),
                });
            } else {
                await route.continue();
            }
        ...
        // 1. Navegar a la pantalla de torneos como invitado
        await page.goto('/tournaments');
        ...
        // 3. Verificaciones: Se ven los públicos, pero el DRAFT sigue estrictamente oculto
        await expect(page.getByText('Open Absoluto de Aragón')).toBeVisible();
        await expect(page.getByText('Liga Secreta en Desarrollo')).not.toBeVisible();
    });
});
